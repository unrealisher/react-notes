...
var localData = data;
var tags = localData.tags,
  colors = localData.colors,
  notes = localData.notes,
  archive = localData.archive;
...
app.get("/api/data", function(req, res) {
  ...
  res.end(
    JSON.stringify({ tags: tags, colors: colors, notes: collection.toArray() })
  );
});
//Заметки
...
app.get("/api/cards/:id", function(req, res) {
  ...
  var note = collection.find(function(item) {
    return item.id.toString() === id;
  });
  if (note) {
...
app.delete("/api/cards/:id", function(req, res) {
  var id = req.params.id;
  var note = collection.toArray().find(function(item) {
    return item.id.toString() === id;
  });
  if (collection.deleteNote(parseInt(id))) {
    if (note) archiveCollection.addNote(note);
    localData = __assign({}, localData, {
...
app.get("/", function(req, res) {
  return res.sendFile(path.resolve(path.resolve("../build/index.html")));
});
